...
class BWidget:
    def __init__(self, content='', prop=None):
    ...
    def to_html(self):
        prop_str = ' '.join(str(p) for p in self.prop + ['reload-from="/_reload"'])
...
class BButton(BWidget):
    ...
    def to_html(self):
        ...
        return f'<button id="{self.id}" {prop_str} data-event="click">{self.content}</button>'


class BTimeWidget(BWidget):
    def to_html(self):
        self.content = f'Time: {time.strftime("%H:%M:%S")}'
        prop_str = ' '.join(self.prop + ['reload-from="/_reload"'])
        return f'<div id="{self.id}" {prop_str}>{self.content}</div>'


class BValueWidget(BWidget):
    def __init__(self, value='', prop=None, onchange=None):
        super().__init__(str(value), prop)
